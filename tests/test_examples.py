...
from examples.vdp import run_vdp, run_vdp_simulate_with_input, run_vdp_optimize_with_input
...
class Test_Optimo_Examples(unittest.TestCase, utilities.partialChecks):
    ...
    def test_vdp_simulate_with_input(self):
        """Test that simulates the Van der Pol oscillator example with external inputs. 
        This example simulates the Van der Pol oscillator model when passing a prescribed
        input trajectory with default initial states. 
        """
        file_ref_sim = os.path.join(root_path, 'tests', 'references', 'vdp_ref_sim_with_input.csv')
        res_sim_df = run_vdp_simulate_with_input(force_recompile=True, plot=False)
        self.compare_ref_timeseries_df(res_sim_df, file_ref_sim, tol=1e-3)

    def test_vdp_optimize_with_input(self):
        """Test that optimizes the Van der Pol oscillator example when prescribing external inputs.
        This example optimizes the Van der Pol oscillator model when setting a prescribed input 
        trajectory with default initial states.
        """
        file_ref_ocp = os.path.join(root_path, 'tests', 'references', 'vdp_ref_ocp_with_input.csv')
        res_ocp_df = run_vdp_optimize_with_input(force_recompile=True, plot=False)
        self.compare_ref_timeseries_df(res_ocp_df, file_ref_ocp, tol=1e-3)
